...
def aggregate_data(df):
    ...
    def calculate_efficiency(df_agg):
        df_agg['cpa'] = (df_agg['cost'] / df_agg['conversions']).where(df_agg['conversions'] > 0, 0)
        df_agg['roas'] = (df_agg['revenue'] / df_agg['cost'] * 100).where(df_agg['cost'] > 0, 0)
        df_agg['ctr'] = (df_agg['clicks'] / df_agg['impressions'] * 100).where(df_agg['impressions'] > 0, 0)
        df_agg['cvr'] = (df_agg['conversions'] / df_agg['clicks'] * 100).where(df_agg['clicks'] > 0, 0)
        return df_agg
...
def update_dashboard_display(df_full_json, month_filter, channel_filter, kpi_type, target_cpa, target_roas):
    ...
    if month_filter != "전체 월" and month_filter != sorted_months[0]:
    ...
    summary = f"🎯 총 광고비 ₩{aggregated_current['overall']['cost']:,.0f}으로 {aggregated_current['overall']['conversions']:,.0f}건의 전환을 달성했습니다. (평균 CPA: ₩{aggregated_current['overall']['cpa']:,.0f}, ROAS: {aggregated_current['overall']['roas']:.1f}%)"
...
body, .gradio-container { font-family: 'Noto Sans KR', sans-serif !important; background-color: var(--background-color) !important; color: var(--text-primary) !important; }
.gradio-container h1 {
...
with gr.Blocks(theme=gr.themes.Soft(primary_hue="blue"), css=css, title="광고 성과 분석 대시보드") as app:
    ...
    analyze_button.click(fn=process_and_init_dashboard, inputs=[file_input, date_col, cost_col, imp_col, click_col, conv_col, channel_col, rev_col, date_format_selector, date_format_custom], outputs=[df_state, dashboard_group, month_filter, channel_filter] + dashboard_components)
